...
impl BowlingGame {
    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        ...
        //let is_fill_ball = self.frame.is_last() && self.throw >= 3 && self.available_pins == 0;
        ...
        if self.throw == self.last_throw || (self.available_pins == 0 && !is_last_frame) {
            // reset available pins for the next frame
            self.frame.next();
            if is_last_frame {
                // In the last frame, if it's an open frame, the game is complete
    ...
    fn process_event(&mut self, pins: u16) {
        if let Some(events) = self.events_queue.0.pop_front() {
            #[cfg(feature = "debug")]
            println!("Processing event: {:?}", events);
            for event in events {
                match event {
